...
public class UserController {
	...
	public ResponseEntity<ApiResponse> login(@Valid @RequestBody UserDto userDto) {
		...
		if (isUser.isPresent()) {
			response = new ApiResponse(AppConstants.SUCCESS, HttpStatus.OK.value(), AppConstants.LOGIN_SUCCESSFULLY);
		} else {
			response = new ApiResponse(AppConstants.FAILURE, HttpStatus.NOT_FOUND.value(), AppConstants.INVALID_LOGIN);
		}
